...
class Insights {
  ...
  loadTransactionsAfterTimestamp = async (timestamp, cacheObject) => {
    ...
    for (let i = 0; i < info.length; i++) {
      ...
      if (args.length > 0) {
        if (Buffer.from(args[0], 'base64').toString() === 'register_name') {
          ...
          if (this.addresses[info[i].sender] !== undefined)
            this.addresses[info[i].sender]++;
          else this.addresses[info[i].sender] = 1;
        } else if (
          Buffer.from(args[0], 'base64').toString() === 'accept_transfer'
        ) {
    ...
    if (info.length > 0) {
      ...
      cacheObject.transactions = cacheObject.transactions.concat(
        info.reverse()
      );
      cacheObject.transactions = cacheObject.transactions.reverse();
  ...
  loadInsights = async () => {
    ...
    for (let i = 0; i < cacheObject.transactions.length && count < 10; i++) {
      try {
        let args =
          cacheObject.transactions[i]['application-transaction'][
            'application-args'
          ];
        ...
        if (args.length > 0) {
          if (Buffer.from(args[0], 'base64').toString() === 'register_name') {
            ...
            Buffer.from(args[0], 'base64').toString() === 'renew_name'
          ) {
    ...
    for (let i in this.addresses) {
      if (this.addresses[i] > maxNamesOwnedByAddress)
        maxNamesOwnedByAddress = this.addresses[i];
    }
